...
- **A playbook is any `.rs` file under `playbooks/` that carries
  ...
  **Mechanism.** The workspace package has one bin target, `src/main.rs`,
  ...
  `rustible init --refresh` regenerates it. Because the generated content is
  deterministic per rustible version, the CLI hashes both files on every run
  and warns when one differs from the generated version ("`src/main.rs` was
  modified; run `rustible init --refresh` or expect breakage"), so a
  well-meaning edit becomes a one-line warning instead of a silent break. The build script walks `playbooks/**/*.rs`, parses each
...
- A **rustible workspace** is a Cargo package whose root also contains
  `rustible.toml`, with the generated, hash-checked shims `src/main.rs` and
  `build.rs` from section 9 and a `src/lib.rs` for code shared across
