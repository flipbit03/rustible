...
#[cfg(test)]
mod ensure_targets_tests {
    use super::*;

    /// A target that is already installed is left alone: no `rustup` runs and
    /// nothing is reported as added.
    #[test]
    fn an_installed_target_is_a_no_op() {
        let Some(installed) = rustup_installed_targets() else {
            return; // no rustup on this machine; nothing to assert
        };
        let Some(have) = installed.first() else {
            return;
        };
        let added = ensure_targets_installed(std::slice::from_ref(have)).unwrap();
        assert!(
            added.is_empty(),
            "reinstalled an installed target: {added:?}"
        );
    }

    /// No triples means no work, and in particular no `rustup` invocation.
    #[test]
