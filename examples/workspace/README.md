...
This folder is a [Rustible](https://github.com/flipbit03/rustible) workspace.
Rustible is a replacement for Ansible: a playbook is an ordinary Rust file
with typed operations, compiled to a static binary, shipped over SSH and run
on the target. The targets need nothing installed — no Python, no agent, no
runtime.
